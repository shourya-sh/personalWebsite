...
import { TerminalSquare, UserSquare2, Code2, GraduationCap, Gamepad2, FileText, FolderOpen, Mail } from 'lucide-react';
    ...
    <nav className="fixed left-0 top-0 w-[240px] h-screen bg-[#0a0a0a] border-r border-[#1f1f1f] flex flex-col justify-between py-6 z-50">
      ...
      <div>
        ...
        <div className="px-6 mb-10 flex items-center gap-3">
          <div className="w-10 h-10 bg-[#00fc40]/10 rounded-md flex items-center justify-center text-[#00fc40] border border-[#00fc40]/20 shrink-0">
            <UserSquare2 size={20} />
          </div>
          <div className="min-w-0">
            <div className="font-display text-[12px] font-bold text-[#00fc40] uppercase tracking-wider truncate">SHOURYA SHETH</div>
            <div className="font-mono text-[9px] text-[#777575] truncate">v2.1.0-core</div>
          </div>
        </div>
      ...
      <div className="mt-auto px-4 py-6 border-t border-[#1f1f1f]">
        <button 
          onClick={() => navigate('/resume')}
          className={`flex items-center gap-3 px-6 py-3 w-full text-left font-display text-[12px] font-bold tracking-widest transition-colors rounded-md group ${location.pathname === '/resume' ? 'bg-[#00fc40]/10 text-[#00fc40]' : 'text-[#777575] hover:bg-[#151515] hover:text-white'}`}
        >
          <FileText size={16} className={location.pathname === '/resume' ? 'text-[#00fc40]' : 'group-hover:text-white transition-colors'} />
          VIEW RESUME
        </button>
      </div>
